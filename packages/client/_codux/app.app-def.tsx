...
const manifestToRouter = (
    ...
    const rootRoute: RouteObject = routes.find((r) => r.id === 'root')!;
    ...
    const routes2 = [rootRoute];

    if (manifest.homeRoute) {
        routes2.push(pageToRoute(manifest.homeRoute, requireModule, setUri, '/'));
    }
    if (manifest.errorRoute) {
        routes2.push(pageToRoute(manifest.errorRoute, requireModule, setUri, 'errors/404'));
    }
    const Router = createRemixStub(routes2);

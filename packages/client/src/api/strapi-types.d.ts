...
export type GetValues<TSchemaUID extends Common.UID.Schema> = {
  [TKey in Attribute.GetOptionalKeys<TSchemaUID>]?: Attribute.Get<TSchemaUID, TKey> extends infer TAttribute extends
    Attribute.Attribute
    ? GetValue<TAttribute>
    : never;
} & {
  [TKey in Attribute.GetRequiredKeys<TSchemaUID>]-?: Attribute.Get<TSchemaUID, TKey> extends infer TAttribute extends
    Attribute.Attribute
    ? GetValue<TAttribute>
    : never;
...
type ComponentValue<TAttribute extends Attribute.Attribute> =
  TAttribute extends Attribute.Component<infer TComponentUID, infer TRepeatable>
    ? IDProperty & Utils.Expression.If<TRepeatable, GetValues<TComponentUID>[], GetValues<TComponentUID>>
    : never;
...
export type GetValue<TAttribute extends Attribute.Attribute> = Utils.Expression.If<
  ...
  Utils.Expression.MatchFirst<
    [
      // Relation
      [Utils.Expression.Extends<TAttribute, Attribute.OfType<'relation'>>, RelationValue<TAttribute>],
      // DynamicZone
      [Utils.Expression.Extends<TAttribute, Attribute.OfType<'dynamiczone'>>, DynamicZoneValue<TAttribute>],
      // Component
      [Utils.Expression.Extends<TAttribute, Attribute.OfType<'component'>>, ComponentValue<TAttribute>],
      // Media

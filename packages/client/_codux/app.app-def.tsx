    ...
    if (page.parentLayouts) {
        for (const layout of page.parentLayouts.reverse()) {
            ...
            routeObject = {
                ...
                path: layout.path, // VLADG new fields needed
                id: layout.id,
            };
        }
    }
...
const manifestToRouter = (
    ...
    const separateRoutes = manifest.routes.map((route) => {
        const path =
            '/' +
            route.path
                .map((part) => {
    ...
    const rootRoute: RouteObject = separateRoutes.find((r) => r.id === 'root')!;
    // unite all child routes under the root route to avoid router exception
    separateRoutes.forEach((route) => {
        if (route === rootRoute) return;

        rootRoute.children = rootRoute.children?.concat(route.children ?? []);
    });
    const unitedRoutes = [rootRoute];

    if (manifest.homeRoute) {
        unitedRoutes.push(pageToRoute(manifest.homeRoute, requireModule, setUri, '/'));
    }
    if (manifest.errorRoute) {
        unitedRoutes.push(pageToRoute(manifest.errorRoute, requireModule, setUri, 'errors/404'));
    }
    const Router = createRemixStub(unitedRoutes);
        ...
        const compute = () => {
            ...
            const routeDirChildren = fs.readdirSync(routeDir).map((name) => {
                ...
                if (isFile && name.endsWith('.tsx')) {
                    ...
                    // [VladG]: added this code to detect folder routes.
                    if (
                        name === '_index' ||
                        name === 'errors' ||
                        name ===
                            'error' /* I am not sure which convention we use for the error page name */
                    ) {
                        return null;
                    }

                    const routeParts = name.split('.');
                    const routePath = fullPath + fs.sep + 'route.tsx';
    ...
    parentLayouts: [
        {
            ...
            path: '/', // VLADG - 2 new fields needed to specify the parent
            id: 'root',
        },
    ],
